...
def transfer_data(source: str, dest: str, num_row: int):
    ...
    with open(source, 'r') as file1, open(dest, 'a' ) as file2:
        for number_row, number in enumerate(file1, 1):
            if number_row == num_row:
                file2.write(number)
                print(f"Строка {num_row} была скопирована из {source} в {dest}")
...
while True:
    ...
    if mode == 1:
        ...
        num_row = int(input("Введите номер строки, которую необходимо скопировать: "))
        print(transfer_data(source, dest, num_row))